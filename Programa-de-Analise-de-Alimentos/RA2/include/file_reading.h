...
int lerCSVpraBIN(const char *arquivoCSV, const char *arquivoBIN) {
    ...
    while (fgets(linha, sizeof(linha), arquivo_csv) != NULL && sucesso) {
        ...
        while (token) {
            ...
            if (campo_atual == 0) {
                ...
                strncpy(alimento.categoria, token, sizeof(alimento.categoria) - 1);
                alimento.categoria[sizeof(alimento.categoria) - 1] = '\0';
            }
...
void verificarArquivoBinario(const char* arquivoBinario) {
    ...
    while (fread(&alimento, sizeof(Alimento), 1, arquivo) == 1) {
        printf("Código: %d | Nome: %s | Calorias: %.2f | Proteínas: %.2f | Categoria: %s\n",
               alimento.codigo, alimento.nome, alimento.calorias, alimento.proteinas, alimento.categoria);
        contador++;
    }
