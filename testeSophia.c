    #include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define MAX 200
#define MAX_DESC 100
#define MAX_CATEGORIAS_SELECIONADAS 10
#define MAX_ALIMENTOS_CATEGORIA 20
#define MIN_ALIMENTOS_CATEGORIA 5
#define MAX_ALIMENTOS 100


// Estrutura das categorias
typedef enum
{
    CEREAIS = 1,
    VERDURAS,
    FRUTAS,
    GORDURAS,
    PESCADOS,
    CARNES,
    LACTEOS,
    BEBIDAS,
    OVOS,
    AÇUCARES,
    MISCELANEAS,
    INDUSTRIALIZADOS,
    PREPARADOS,
    LEGUMINOSAS,
    SEMENTES,
} Categoria;

// Estrutra dos alimentos
typedef struct
{
    int numero_do_alimento;
    char descricao[MAX_DESC];
    float umidade;
    float energia;
    float proteina;
    float carboidrato;
    Categoria categoria;
} Alimento;

// Array global para armazenar os alimentos
Alimento alimentos[MAX_ALIMENTOS];
int total_alimentos = 0;

// Função para remover quebras de linha
void removerQuebraLinha(char *str) {
    int len = strlen(str);
    if (len > 0 && str[len-1] == '\n') {
        str[len-1] = '\0';
    }
    if (len > 1 && str[len-2] == '\r') {
        str[len-2] = '\0';
    }
}

// Função para converter o inteiro recebido para  uma das categorias do ENUM
Categoria intParaCategoria(int cat_num) {
    switch(cat_num) {
        case 1: return CEREAIS;
        case 2: return VERDURAS;
        case 3: return FRUTAS;
        case 4: return GORDURAS;
        case 5: return PESCADOS;
        case 6: return CARNES;
        case 7: return LACTEOS;
        case 8: return BEBIDAS;
        case 9: return OVOS;
        case 10: return AÇUCARES;
        case 11: return MISCELANEAS;
        case 12: return INDUSTRIALIZADOS;
        case 13: return PREPARADOS;
        case 14: return LEGUMINOSAS;
        case 15: return SEMENTES;
        default: return CEREAIS;
    }
}

// Função principal para ler o arquivo CSV
int lerArquivoCSV(const char * listadealimentos) {
    FILE *arquivo = fopen(listadealimentos, "r");
    if (arquivo == NULL) {
        printf("Erro: Não foi possível abrir o arquivo %s\n", listadealimentos);
        return 0;
    }

    char linha[500];
    total_alimentos = 0;
    
    // Pular o cabeçalho (primeira linha)
    if (fgets(linha, sizeof(linha), arquivo) == NULL) {
        printf("Erro: Arquivo vazio\n");
        fclose(arquivo);
        return 0;
    }

    // Ler cada linha do arquivo
    while (fgets(linha, sizeof(linha), arquivo) && total_alimentos < MAX_ALIMENTOS) {
        removerQuebraLinha(linha);
        
        // Usar strtok para separar os campos por vírgula
        char *token = strtok(linha, ",");
        if (token == NULL) continue;
        
        // Número do alimento
        alimentos[total_alimentos].numero_do_alimento = atoi(token);
        
        // Descrição
        token = strtok(NULL, ",");
        if (token == NULL) continue;
        strncpy(alimentos[total_alimentos].descricao, token, MAX_DESC - 1);
        alimentos[total_alimentos].descricao[MAX_DESC - 1] = '\0';
        
        
        // Umidade
        token = strtok(NULL, ",");
        if (token == NULL) continue;
        alimentos[total_alimentos].umidade = atof(token);
        
        // Energia
        token = strtok(NULL, ",");
        if (token == NULL) continue;
        alimentos[total_alimentos].energia = atof(token);
        
        // Proteína
        token = strtok(NULL, ",");
        if (token == NULL) continue;
        alimentos[total_alimentos].proteina = atof(token);
        
        // Carboidrato
        token = strtok(NULL, ",");
        if (token == NULL) continue;
        alimentos[total_alimentos].carboidrato = atof(token);
        
        // Categoria
        token = strtok(NULL, ",");
        if (token == NULL) continue;
        alimentos[total_alimentos].categoria = intParaCategoria(atoi(token));
        
        total_alimentos++;
    }

    fclose(arquivo);
    printf("Arquivo carregado com sucesso! %d alimentos lidos.\n", total_alimentos);
    return 1;
}
void listarCategorias()
{
    printf("\n1. Cereais e derivados");
    printf("\n2. Verduras, hortaliças e derivados");
    printf("\n3. Frutas e derivados");
    printf("\n4. Gorduras e óleos");
    printf("\n5. Pescados e frutos do mar");
    printf("\n6. Carnes e derivados");
    printf("\n7. Leite e derivados");
    printf("\n8. Bebidas(alcoólicas e não alcoólicas)");
    printf("\n9. Ovos e derivados");
    printf("\n10. Produtos açucarados");
    printf("\n11. Miscelâneas");
    printf("\n12. Outros alimentos industrializados");
    printf("\n13. Alimentos preparados");
    printf("\n14. Leguminosas e derivados");
    printf("\n15. Nozes e sementes\n");
}
int filtrarPorCategoria(Categoria cat, Alimento resultado[]) {
    // Percorre todos os alimentos
    // Se categoria coincide, adiciona ao array resultado
    // Retorna quantos alimentos foram encontrados
}
int main()
{
    if (!lerArquivoCSV("lista_de_alimentos.csv")) {
    printf("Erro ao carregar o arquivo. Encerrando programa.\n");
    return 1;
    }
    int opcao;
    do
    {
        printf("\n--- MENU ---\n");
        printf("\n(1) - Listar as categorias de alimentos");
        printf("\n(2) - Listar todos os alimentos de certa categoria em ordem alfabética de acordo com a descrição");
        printf("\n(3) - Listar todos os alimentos de certa categoria em ordem decrescente de acordo com a energia(Kcal)");
        printf("\n(4) - Listar os alimentos de certa categoria em ordem decrescente de acordo com a umidade(%%)");
        printf("\n(5) - Listar os alimentos de certa categoria em ordem decrescente de acordo com a energia(Kcal)");
        printf("\n(6) - Listar os alimentos de certa categoria em ordem decrescente de acordo com a proteína(g)");
        printf("\n(7) - Listar os alimentos de certa categoria em ordem decrescente de acordo com a carboidrato(g)");
        printf("\n(8) - Listar os alimentos de certa categoria em ordem decrescente de acordo com a relação de energia(Kcal) e proteína(g)");
        printf("\n(9) - Listar os alimentos de certa categoria em ordem decrescente de acordo com a relação de energia(Kcal) e carboidrato(g)");
        printf("\n(10) - Encerrar o programa");
        printf("\nOpção: ");
        scanf("%d", &opcao);

        if (opcao == 1)
        {
            listarCategorias();
        }
        else if (opcao >= 2 && opcao <= 9)
        {
            int cat, N;
            listarCategorias();
            printf("\nEscolha uma categoria: ");
            scanf("%d", &cat);
        }

    } while (opcao != 10);

    return 0;
}